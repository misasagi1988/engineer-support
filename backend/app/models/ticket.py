...
class Ticket(Base):
    ...
    module_id: Mapped[str] = mapped_column(String(36), ForeignKey("modules.id"), nullable=True, index=True)
    ...
    status: Mapped[str] = mapped_column(SAEnum("pending", "processing", "resolved", "closed"), default="pending", index=True)
    priority: Mapped[str] = mapped_column(SAEnum("p0", "p1", "p2", "p3"), default="p2", index=True)
    assignee_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id"), nullable=True, index=True)
