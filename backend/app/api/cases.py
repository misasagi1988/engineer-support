...
async def create_case(data: CaseCreate, db: AsyncSession = Depends(get_db)):
    return await case_service.create_case(db, data, created_by="system")
...
async def review_case(case_id: str, data: CaseReview, db: AsyncSession = Depends(get_db)):
    case = await case_service.get_case(db, case_id)
    if not case:
        raise HTTPException(status_code=404, detail="Case not found")
    await case_service.review_case(db, case, data.review_status, reviewed_by="admin")
    await db.commit()
    await db.refresh(case)
    return case
